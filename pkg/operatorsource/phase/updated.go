...
func NewUpdatedEventReconciler(logger *log.Entry, datastore datastore.Writer, kubeclient kube.Client) Reconciler {
...
func (r *updatedEventReconciler) Reconcile(ctx context.Context, in *v1alpha1.OperatorSource) (out *v1alpha1.OperatorSource, nextPhase *NextPhase, err error) {
	out = in.DeepCopy()

	r.logger.Info("Spec has changed, purging all resource(s) associated with it")

	r.datastore.Remove(in.GetUID())
	...
	if err = r.kubeclient.Delete(csc); err != nil && !k8s_errors.IsNotFound(err) {
	...
	r.logger.Info("Spec has changed, scheduling for validation")
