...
func (h *operatorsourcehandler) Handle(ctx context.Context, in *v1alpha1.OperatorSource) error {
	...
	// The pre-phase reconciler has handled the event. So we should transition
	// into the new phase and return.
	if status != nil {
		return h.transition(ctx, logger, out, status, err)
	}

	// The pre-phase reconciler has not handled the event, so let's use the
	// regular phase reconciler to handle this event.
