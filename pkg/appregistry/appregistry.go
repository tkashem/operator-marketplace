...
func (f *factory) New(options Options) (Client, error) {
	...
	// If a bearer token has been specified then we shpu
	if options.AuthToken != "" {
		bearerTokenAuthWriter := httptransport.BearerToken(options.AuthToken)
		transport.DefaultAuthentication = bearerTokenAuthWriter
	}
